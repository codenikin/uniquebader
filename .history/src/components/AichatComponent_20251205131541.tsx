      ...
      <AnimatePresence>
        {showChatIcon && (
          ...
          >
            <button
              onClick={toggleChat}
              className="bg-gray-600 hover:bg-blue-700 text-white rounded-full p-4 shadow-lg transition-all"
            >
              <img src="/chatbot.png" alt="Close" className="w-5 h-5" />
              {!isChatOpen ? <div className="" /> : <FiArrowDownCircle className="size-6" />}
            </button>
          </motion.div>
        )}
      </AnimatePresence>
      <AnimatePresence>
        {isChatOpen && (
          <motion.div
            ...
            transition={{ duration: 0.2 }}
            style={{
              height: window.innerWidth < 768 ? 'calc(100vh - 250px)' : 'calc(100vh - 200px)',
            }}
