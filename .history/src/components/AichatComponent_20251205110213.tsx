      ...
      <AnimatePresence>
        {showChatIcon && (
          ...
          >
            ...
            >
              {!isChatOpen ? <FiMessageCircle className="size-6" /> : <FiArrowDownCircle className="size-6" />}
            </button>
          </motion.div>
        )}
      </AnimatePresence>
      <AnimatePresence>
        {isChatOpen && (
          <motion.div
            ...
            transition={{ duration: 0.2 }}
            className="fixed bottom-20 right-4 z-50 md:w-[500px] w-[90vw] h-[600px] flex flex-col"
          >
            <Card className="border-2 h-full flex flex-col shadow-xl rounded-lg">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 border-b">
                <CardTitle className="text-lg font-semibold text-gray-800">Chat Assistant</CardTitle>
                <button
                  onClick={toggleChat}
              ...
              <CardContent className="flex-1 overflow-y-auto pr-3 py-4">
                {messages?.length === 0 && (
                      ...
                      <ReactMarkdown
                        components={{
                          code: ({ inline, children, className }: CodeProps) =>
                            inline ? (
                              <code className={`${message.role === 'user' ? 'bg-blue-500' : 'bg-gray-300'} px-1 rounded text-xs font-mono`}>
                                {children}
                              </code>
                            ) : (
                              <pre className={`${message.role === 'user' ? 'bg-blue-500' : 'bg-gray-300'} rounded p-2 overflow-x-auto text-xs font-mono my-2`}>
                          ...
                          p: ({ children }: ParagraphProps) => {
                            ...
                            return hasBlock ? <div>{children}</div> : <p className="text-sm">{children}</p>
                          },
                ...
                {isLoading && (
                  <div className="w-full items-center flex justify-center gap-x-3 py-3">
                    ...
                    <button className="underline text-sm text-gray-600 hover:text-gray-800" type="button" onClick={() => stop()}>
                      Abort
                    </button>
                  </div>
                )}
                {error && (
                  <div className="w-full text-red-600 items-center justify-center flex gap-3 py-3 bg-red-50 rounded p-3">
                    ...
                    <button className="underline text-sm hover:text-red-800" type="button" onClick={() => reload()}>
                      Retry
                    </button>
                  </div>
                )}
